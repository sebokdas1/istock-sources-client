  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-purple-500">
        Your Total Order is {orders.length}
      </h2>
            ...
            {orders.map((ap, index) => (
              <tr key={ap._id}>
                ...
                <td className="card-actions">
                  {!ap?.paid && (
                    <button className="btn btn-xs bg-red-500 text-white cancel-modal">
                      <label
                ...
                <td>
                  {ap.price && !ap.paid && (
                    <Link to={`/dashboard/payment/${ap._id}`}>
                      <button className="btn btn-xs btn-success">pay</button>
                    </Link>
                  )}
                  {ap.price && ap.paid && (
                    <div
                      className="tooltip text-white flex gap-1 items-center"
                      data-tip={ap.trxId}
                    >
                      <span className=" text-success">Paid</span>
                      <ClickToCopy textToCopy={ap.trxId} />
                    </div>
                  )}
                </td>
              </tr>
